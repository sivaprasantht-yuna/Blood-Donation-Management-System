...
import { ShieldCheck, Landmark, Users, Heart, Clipboard, Check, Trash2, Key, AlertCircle, Plus, Sparkles, MapPin, Search, Mail, MessageSquare, Clock } from 'lucide-react';
import { BloodRequest, DonationCamp, DonationHistory } from '../types';
...
export default function DashboardAdmin({ onLogout, liveTrigger }: DashboardAdminProps) {
  ...
  const handleApproveHospital = async (id: string, name: string) => {
    try {
      ...
      if (response.ok) {
        setHospitals(prev => prev.map(h => h.id === id ? { ...h, isApproved: true } : h));
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-[#111827] text-gray-400">
        <div className="text-center space-y-2">
          ...
          <p className="text-sm font-semibold">Resolving Command Center metrics...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto space-y-8 text-left">
        ...
        <div className="bg-white dark:bg-[#1F2937] rounded-3xl p-6 md:p-8 border border-gray-150 dark:border-gray-800 shadow-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative overflow-hidden transition-colors">
          ...
          <div className="flex items-center gap-5 relative z-10">
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <h2 className="text-2xl font-black tracking-tight text-gray-900 dark:text-white">Admin Command Center</h2>
                <span className="text-[10px] bg-red-150 dark:bg-red-950 text-red-650 dark:text-red-400 font-black px-2.5 py-1 rounded border border-red-200 dark:border-red-900 uppercase">SYS SECURE</span>
              </div>
              <p className="text-xs text-gray-500 dark:text-gray-300">Oversee vetted hospitals, donor records passports, and audit live emergency request streams.</p>
            </div>
          </div>
          ...
          <div className="text-xs text-gray-400 font-mono font-bold bg-gray-50 dark:bg-gray-800 p-3.5 rounded-2xl border border-gray-100 dark:border-gray-700 self-stretch md:self-auto flex md:flex-col justify-between md:justify-center items-center md:items-start gap-1">
            ...
            <span className="text-gray-901 dark:text-white font-semibold">administrator@lifedrop.in</span>
          </div>
        </div>
          ...
          <div className="space-y-8 animate-in fade-in duration-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="bg-white dark:bg-[#1F2937] rounded-3xl p-6 border border-gray-150 dark:border-gray-800 shadow-md flex items-center justify-between transition-colors">
                <div className="space-y-1">
                  <span className="text-[10px] font-bold text-gray-400 dark:text-gray-300 uppercase tracking-widest">Active Donors</span>
                  <p className="text-4xl font-black text-gray-900 dark:text-white tracking-tight leading-none">{stats.totalDonors}</p>
                  <span className="text-[11px] text-gray-500 dark:text-gray-300 block font-semibold">Pre-screened passports</span>
                </div>
              ...
              <div className="bg-white dark:bg-[#1F2937] rounded-3xl p-6 border border-gray-150 dark:border-gray-800 shadow-md flex items-center justify-between transition-colors">
                <div className="space-y-1">
                  <span className="text-[10px] font-bold text-gray-400 dark:text-gray-300 uppercase tracking-widest">Hospitals Hub</span>
                  <p className="text-4xl font-black text-gray-900 dark:text-white tracking-tight leading-none">{stats.totalHospitals}</p>
                  <span className="text-[11px] text-gray-500 dark:text-gray-300 block font-semibold">{pendingApprovals.length} Licenses pending</span>
                </div>
              ...
              <div className="bg-white dark:bg-[#1F2937] rounded-3xl p-6 border border-gray-150 dark:border-gray-800 shadow-md flex items-center justify-between transition-colors">
                <div className="space-y-1">
                  <span className="text-[10px] font-bold text-gray-400 dark:text-gray-300 uppercase tracking-widest">Total Matches</span>
                  <p className="text-4xl font-black text-gray-900 dark:text-white tracking-tight leading-none">{stats.totalRequests}</p>
                  <span className="text-[11px] text-gray-500 dark:text-gray-300 block font-semibold">{stats.completedRequests} Fulfilled and certified</span>
                </div>
              ...
              <div className="bg-white dark:bg-[#1F2937] rounded-3xl p-6 border border-gray-150 dark:border-gray-800 shadow-md flex items-center justify-between transition-colors">
                <div className="space-y-1">
                  <span className="text-[10px] font-bold text-[#DC2626] uppercase tracking-widest">Lives Saved Scoring</span>
                  <p className="text-4xl font-black text-red-650 dark:text-red-400 tracking-tight leading-none">{stats.livesSaved}</p>
                  <span className="text-[11px] text-gray-500 dark:text-gray-300 block font-semibold">Active transfusions registered</span>
                </div>
            ...
            {pendingApprovals.length > 0 && (
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-extrabold text-gray-900 dark:text-white text-lg">Hospital Licensing Credentials Pending Approval ({pendingApprovals.length})</h3>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {pendingApprovals.map((hosp) => (
                    ...
                    >
                      <div>
                        <h4 className="font-extrabold text-sm text-gray-905 dark:text-white leading-tight">{hosp.hospitalName}</h4>
                        <p className="text-[10px] text-gray-400 mt-0.5 font-bold uppercase tracking-wider">{hosp.city} sector</p>
                      </div>

                      <div className="space-y-1.5 text-xs text-gray-500 dark:text-gray-300">
                        <p className="flex justify-between">
                          <span>License No:</span>
                          <span className="font-mono font-bold text-gray-700 dark:text-white">{hosp.licenseNumber}</span>
                        </p>
                        <p className="flex justify-between">
                          <span>Admin Contact:</span>
                          <span className="font-semibold text-gray-700 dark:text-white">{hosp.phone}</span>
                        </p>
                        <p className="flex justify-between">
                          <span>Email Hub:</span>
                          <span className="font-semibold text-gray-700 dark:text-white break-all">{hosp.email}</span>
                        </p>
                      </div>

                      <div className="flex gap-2 pt-2">
                        <button
                          onClick={() => handleApproveHospital(hosp.id, hosp.hospitalName)}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 text-left">
              ...
              <div className="bg-white dark:bg-[#1F2937] p-6 rounded-3xl border border-gray-150 dark:border-gray-800 shadow-xl space-y-4 transition-colors">
                <h3 className="font-bold text-gray-905 dark:text-white text-lg">Hospital Operations Hubs ({hospitals.length})</h3>
                
                <div className="divide-y divide-gray-50 dark:divide-gray-750 max-h-[400px] overflow-y-auto pr-2">
                  {hospitals.map((hosp) => (
                    <div key={hosp.id} className="py-3 flex justify-between items-center gap-4">
                      <div>
                        <h4 className="font-extrabold text-sm text-gray-901 dark:text-white">{hosp.hospitalName}</h4>
                        <p className="text-[10px] text-gray-450 dark:text-gray-300 font-bold">{hosp.city} sector • Code: {hosp.licenseNumber}</p>
                      </div>
                      
                      <div className="flex items-center gap-2">
                        {hosp.isApproved ? (
                          <span className="text-[9px] bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-450 font-black px-2.5 py-1 rounded border border-emerald-100 dark:border-emerald-900 uppercase">VETTED ACTIVE</span>
                        ) : (
                          <span className="text-[9px] bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-405 font-bold px-2 py-1 rounded border border-amber-100 dark:border-amber-900 uppercase">PENDING APPROVAL</span>
                        )}
                        <button
                          onClick={() => handleDeleteHospital(hosp.id, hosp.hospitalName)}
              ...
              <div className="bg-white dark:bg-[#1F2937] p-6 rounded-3xl border border-gray-150 dark:border-gray-800 shadow-xl space-y-4 transition-colors">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2.5">
                  <h3 className="font-bold text-gray-901 dark:text-white text-lg">Vetted Donor Passports ({filteredDonors.length})</h3>
                ...
                <div className="divide-y divide-gray-50 dark:divide-gray-750 max-h-[400px] overflow-y-auto pr-2">
                  {filteredDonors.map((donor) => (
                    <div key={donor.id} className="py-3 flex justify-between items-center gap-4">
                      <div className="flex items-center gap-2.5">
                        <span className="w-8 h-8 rounded-lg bg-red-100 dark:bg-red-950 text-red-650 dark:text-red-400 font-black text-xs flex items-center justify-center uppercase">{donor.bloodGroup}</span>
                        <div>
                          <h4 className="font-bold text-xs text-gray-901 dark:text-white">{donor.fullName}</h4>
                          <p className="text-[10px] text-gray-450 dark:text-gray-300 font-semibold">{donor.city} • Impact score: {donor.totalDonations} units</p>
                        </div>
                      </div>

                      <div className="flex items-center gap-2">
                        {donor.isAvailable ? (
                          <span className="text-[9px] text-emerald-600 dark:text-emerald-450 font-bold uppercase bg-emerald-50 dark:bg-emerald-950/40 px-2 py-0.5 rounded">Active ready</span>
                        ) : (
                          <span className="text-[9px] text-gray-400 dark:text-gray-500 font-bold uppercase bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded">Resting</span>
                        )}
                        <button
        ...
        {activeTab === 'audit' && (
          <div className="space-y-6 animate-in fade-in duration-200">
            ...
            <div className="bg-white dark:bg-[#1F2937] p-6 rounded-3xl border border-gray-150 dark:border-gray-800 shadow-xl text-left space-y-4 transition-colors">
              <div>
                <h3 className="font-black text-gray-901 dark:text-white text-lg">Vetted System Logs Core</h3>
                <p className="text-xs text-gray-500 dark:text-gray-350">Systematic administrative log trace for regulatory auditing. Tracks matching acceptances, security settings modifications, credentials verification states, and communications streams.</p>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs text-left text-gray-505 dark:text-gray-300">
                  <thead className="text-[10px] text-gray-400 dark:text-gray-300 uppercase tracking-widest border-b border-gray-50 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/40">
                    <tr>
                      <th scope="col" className="px-6 py-3 font-semibold">Author level</th>
                      <th scope="col" className="px-6 py-3 font-semibold">Action label</th>
                      <th scope="col" className="px-6 py-3 font-semibold">Details description</th>
                  ...
                  <tbody className="divide-y divide-gray-50 dark:divide-gray-750">
                    {activityLogs.length === 0 ? (
                      <tr>
                        <td colSpan={4} className="py-8 text-center text-gray-300">Quiet server core. No activity alerts logged.</td>
                      </tr>
                    ) : (
                      activityLogs.slice(0, 100).map((log) => (
                        <tr key={log.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/30">
                          ...
                          <td className="px-6 py-3.5 font-bold font-mono text-[10px] text-gray-900 dark:text-white">{log.action}</td>
                          <td className="px-6 py-3.5 text-gray-650 dark:text-gray-305 leading-relaxed">{log.description}</td>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
              ...
              <div className="bg-white dark:bg-[#1F2937] p-6 rounded-3xl border border-gray-150 dark:border-gray-800 shadow-xl space-y-3 transition-colors">
                ...
                <p className="text-[11px] text-gray-400 leading-normal">Systematic trace registry of outgoing mock clinical email transmissions.</p>
                
                <div className="space-y-3 max-h-[350px] overflow-y-auto divide-y divide-gray-50 dark:divide-gray-750 pr-1 text-xs">
                  {emailLogs.length === 0 ? (
                    <p className="text-center text-gray-400 dark:text-gray-300 py-8">No email packets sent currently.</p>
                  ) : (
                    emailLogs.slice(0, 50).map((mail) => (
                      <div key={mail.id} className="pt-3 first:pt-0">
                        ...
                        <h5 className="font-black text-gray-901 dark:text-white">Subject: {mail.subject}</h5>
              ...
              <div className="bg-white dark:bg-[#1F2937] p-6 rounded-3xl border border-gray-150 dark:border-gray-800 shadow-xl space-y-3 transition-colors">
                ...
                <p className="text-[11px] text-gray-400 leading-normal">Systematic log registry of SMS text broadcasts pushed over mock devices.</p>
                
                <div className="space-y-3 max-h-[350px] overflow-y-auto divide-y divide-gray-50 dark:divide-gray-750 pr-1 text-xs">
                  {smsLogs.length === 0 ? (
                    <p className="text-center text-gray-400 dark:text-gray-300 py-8">No SMS text broadcasts pushed of yet.</p>
                  ) : (
                    smsLogs.slice(0, 50).map((sms) => (
                      <div key={sms.id} className="pt-3 first:pt-0">
                        <div className="flex justify-between items-center mb-1 text-[10px] text-gray-450 dark:text-gray-350">
                          <span>Phone: <strong>{sms.to}</strong></span>
                          <span className="text-emerald-500 font-bold uppercase text-[9px]">{sms.status}</span>
                        </div>
