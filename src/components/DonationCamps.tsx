...
import { Calendar, Clock, MapPin, Phone, User, Plus, Check } from 'lucide-react';
import { motion } from 'motion/react';

interface DonationCampsProps {
  camps: DonationCamp[];
...
export default function DonationCamps({ camps, onAddCamp, isAuthenticated, userType }: DonationCampsProps) {
  ...
  const handleCopy = (id: string, contact: string) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.title || !formData.location || !formData.city || !formData.date || !formData.time || !formData.organizer) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-2xl font-bold text-gray-900 tracking-tight">Active Donation Camps</h3>
          <p className="text-sm text-gray-500 mt-1">Join local donation campaigns happening in major cities near you.</p>
        </div>
        
        {/* Only hospitals or admins can host a new camp */}
        {isAuthenticated && (userType === 'hospital' || userType === 'admin') && (
          <button
      ...
      {showAddForm && (
        ...
        >
          <h4 className="text-lg font-bold text-gray-900 mb-4">Register a Donation Camp</h4>
          {successMsg && (
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1">Camp Title *</label>
              <input
                type="text"
                required
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1">Camp Location Address *</label>
              <input
                type="text"
                required
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1">Organizer Hospital/NGO *</label>
              <input
                type="text"
                required
            ...
            <div className="md:col-span-2">
              <label className="block text-xs font-semibold text-gray-600 mb-1">Inquiries Contact Phone</label>
              <input
                type="tel"
          ...
          {camps.map((camp) => (
            ...
            >
              <div>
                ...
                <h4 className="font-bold text-gray-900 text-lg leading-snug line-clamp-2">{camp.title}</h4>
                ...
                <div className="space-y-2.5 mt-4 text-sm text-gray-600">
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="text-xs truncate">Hosted by: <strong className="font-semibold text-gray-700">{camp.organizer}</strong></span>
                  </div>
                </div>
              </div>
              ...
              {camp.contact && (
                <div className="mt-5 pt-4 border-t border-gray-50 flex items-center justify-between">
                  <span className="text-xs font-semibold text-gray-500">Inquiries:</span>
                  <button
