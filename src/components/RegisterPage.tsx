...
import { Heart, Landmark, MapPin, User, ShieldAlert, Check, ChevronRight, ArrowLeft } from 'lucide-react';

interface RegisterPageProps {
  onRegisterDonorSuccess: (token: string, user: any) => void;
  onRegisterHospitalSuccess: () => void;
  onGoToLogin: () => void;
  onGoToHome: () => void;
}
...
export default function RegisterPage({
  ...
  const handleDonorSubmit = async (e: React.FormEvent) => {
    ...
    // Validations
    if (!donorForm.fullName || !donorForm.age || !donorForm.phoneNumber || !donorForm.email || !donorForm.password || !donorForm.confirmPassword || !donorForm.address || !donorForm.city || !donorForm.state) {
    ...
    if (!indianPhoneRegex.test(cleanPhone)) {
      setErrorMsg("Please enter a valid Indian mobile number (e.g. 9876543210, with optional +91 prefix).");
      return;
    }
  ...
  const handleHospitalSubmit = async (e: React.FormEvent) => {
    ...
    if (!hospitalForm.hospitalName || !hospitalForm.licenseNumber || !hospitalForm.email || !hospitalForm.phone || !hospitalForm.address || !hospitalForm.city || !hospitalForm.password || !hospitalForm.confirmPassword) {
    ...
    try {
      ...
      setSuccessMsg("Application received! Admin approval is required before signing in.");
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className="text-center sm:text-right">
            <p className="text-xs text-gray-400">
              ...
              <button onClick={onGoToLogin} className="text-red-650 hover:text-red-700 font-bold hover:underline">
                Sign In Instead
              </button>
            </p>
          </div>
          ...
          <div className="p-6 md:p-10 space-y-6">
            ...
            <div className="text-center max-w-xl mx-auto space-y-1.5">
              <h3 className="text-2xl font-extrabold text-gray-900 tracking-tight">
                {activeTab === 'donor' ? 'Become a Certified Hero' : 'Register Clinical Facility'}
              </h3>
            ...
            {/* DONOR FORM LAYOUT */}
            {activeTab === 'donor' && (
              <form onSubmit={handleDonorSubmit} className="space-y-6 text-left">
                
                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest border-b border-gray-50 pb-2">Personal Particulars</h4>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                  ...
                  <div className="space-y-1">
                    <label className="block text-xs font-semibold text-gray-600">Age (18 - 65) *</label>
                    <input
                      type="number"
                      required
                      min={18}
                ...
                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest border-b border-gray-50 pb-2">Medical Profile</h4>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                  <div className="space-y-1">
                    <label className="block text-xs font-semibold text-gray-600">Blood Group *</label>
                    <select
                      id="donor-reg-blood"
                  ...
                  <div className="space-y-1">
                    <label className="block text-xs font-semibold text-gray-600">Last Donation Date</label>
                    <input
                      type="date"
                ...
                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest border-b border-gray-50 pb-2">Full Address & Credentials</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1">
                      <label className="block text-xs font-semibold text-gray-600">State *</label>
                      <input
                        type="text"
                        required
                ...
                <div className="pt-4">
                  ...
                  >
                    {loading ? "Creating Account..." : "Finalize Profile Registration"}
            ...
            {activeTab === 'hospital' && (
              <form onSubmit={handleHospitalSubmit} className="space-y-6 text-left">
                
                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest border-b border-gray-50 pb-2">Medical Facility License</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="space-y-1">
                    <label className="block text-xs font-semibold text-gray-600">Hospital Name *</label>
                    <input
                      type="text"
                      required
                  ...
                  <div className="space-y-1">
                    <label className="block text-xs font-semibold text-gray-600">Accredited License Code *</label>
                    <input
                      type="text"
                      required
                ...
                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest border-b border-gray-50 pb-2">Clinical Location & Logistics</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="space-y-1">
                    <label className="block text-xs font-semibold text-gray-600">Street Address *</label>
                    <input
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="block text-xs font-semibold text-gray-600">City / District *</label>
                      <input
                        type="text"
                        required
                        placeholder="e.g. CHENNAI"
                    ...
                    <div className="space-y-1">
                      <label className="block text-xs font-semibold text-gray-600">Duty Contact Phone *</label>
                      <input
                        type="tel"
                        required
                ...
                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest border-b border-gray-50 pb-2">Login Credentials</h4>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                  <div className="space-y-1">
                    <label className="block text-xs font-semibold text-gray-600">Contact Email Address *</label>
                    <input
                      type="email"
                      required
                ...
                <div className="pt-4 border-t border-gray-100 flex flex-col md:flex-row items-center gap-4">
                  ...
                  >
                    {loading ? "Submitting Application..." : "Apply Clinical Sign Up"}
                    <ChevronRight size={16} />
                  </button>
                  <span className="text-[11px] text-amber-600 bg-amber-50 px-3 py-1.5 rounded-lg flex items-center gap-1.5 font-semibold">
                    <ShieldAlert size={14} /> Hospital accounts must undergo administrative license verification.
                  </span>
                </div>
