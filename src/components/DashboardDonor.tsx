...
import { User, Heart, Shield, Award, Calendar, Check, AlertCircle, Copy, CheckCircle, Bell, Sparkles, MapPin, Eye, ToggleLeft, ToggleRight, Download, Mail, MessageSquare, Settings, Clock, Landmark } from 'lucide-react';
import CertificateModal from './CertificateModal';
import { BloodRequest, DonationHistory, Notification } from '../types';
...
export default function DashboardDonor({ donorId, onLogout, liveTrigger }: DashboardDonorProps) {
  ...
  const [activeTab, setActiveTab] = useState<'hub' | 'comms' | 'settings'>('hub');
  ...
  const fetchDonorData = async () => {
    try {
      ...
      if (reqRes.ok) {
        const allRequests: BloodRequest[] = await reqRes.json();
      ...
      if (logsRes.ok) {
        ...
        if (profData) {
          setEmailLogs(commData.emailLogs.filter((m: any) => m.to === profData.email));
          setSmsLogs(commData.smsLogs.filter((s: any) => s.to === profData.phoneNumber));
        }
      }
  ...
  const handleToggleAvailability = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setProfile((prev: any) => ({ ...prev, isAvailable: updated.isAvailable }));
        setToastMsg(`Safety status modified: You are now ${updated.isAvailable ? 'Available for matches' : 'Resting/Unavailable'}`);
  ...
  const handleAcceptAlert = async (requestId: string) => {
    setAcceptingId(requestId);
    try {
      const response = await fetch(`/api/donors/${donorId}/accept/${requestId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      });
      ...
      setToastMsg("Success! Alert accepted. Hospital has been notified with your Indian mobile number!");
  ...
  const handleSaveSettings = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setToastMsg("Notification settings successfully saved to profile database!");
        setTimeout(() => setToastMsg(""), 2000);
        fetchDonorData();
      }
    } catch (err) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <AnimatePresence>
          {toastMsg && (
            ...
            >
              <Sparkles size={18} className="text-red-400 animate-pulse animate-duration-1000" />
        ...
        <div className="bg-white dark:bg-[#1F2937] rounded-3xl p-6 md:p-8 border border-gray-150 dark:border-gray-800 shadow-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative overflow-hidden transition-colors">
          ...
          <div className="flex items-center gap-5 relative z-10">
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2 flex-wrap">
                <h2 className="text-2xl font-black text-gray-900 dark:text-white tracking-tight">{profile.fullName}</h2>
          ...
          <div className="self-stretch md:self-auto bg-gray-50 dark:bg-gray-800/40 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 flex items-center justify-between gap-5">
            <div className="text-left space-y-0.5">
              <span className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest">Matching Switch</span>
              <span className="text-xs font-black text-gray-800 dark:text-white">
                {profile.isAvailable ? "🟢 Available Drive Ready" : "🔴 Resting / Unavailable"}
              </span>
            </div>
            <button
              ...
              style={{ backgroundColor: profile.isAvailable ? '#10B981' : '#D1D5DB' }}
          ...
          <div className="space-y-8 animate-in fade-in duration-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <div className="bg-white dark:bg-[#1F2937] rounded-3xl p-6 border border-gray-150 dark:border-gray-800 shadow-md flex flex-col justify-between transition-colors">
                <span className="text-[10px] font-bold text-gray-400 dark:text-gray-300 uppercase tracking-widest">Total Drive Donations</span>
                <div className="my-4 text-left">
                  <p className="text-4xl font-black text-gray-900 dark:text-white tracking-tight leading-none">{profile.totalDonations} Units</p>
                  <p className="text-xs font-bold text-gray-800 dark:text-gray-250 mt-1">Sustained Donations</p>
                </div>
                <p className="text-[10px] text-gray-400 dark:text-gray-300 leading-normal">Registered and certified in hospital portal.</p>
              </div>

              <div className="bg-white dark:bg-[#1F2937] rounded-3xl p-6 border border-gray-150 dark:border-gray-800 shadow-md flex flex-col justify-between transition-colors">
                <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest">Lives Saved Scorecard</span>
                <div className="my-4 text-left">
                  <p className="text-4xl font-black text-emerald-600 dark:text-emerald-400 tracking-tight leading-none">{profile.livesSaved} Lives</p>
                  <p className="text-xs font-bold text-gray-800 dark:text-gray-250 mt-1">Impact scorecard</p>
                </div>
                <p className="text-[10px] text-gray-400 dark:text-gray-300 leading-normal">Each donation assists 3 critical patients.</p>
              </div>
              ...
              <div className="bg-white dark:bg-[#1F2937] p-6 rounded-3xl border border-gray-150 dark:border-gray-800 shadow-md flex flex-col justify-between transition-colors">
                <span className={`text-[10px] font-bold uppercase tracking-widest ${isEligible ? 'text-red-500' : 'text-amber-500'}`}>
                  {isEligible ? "Drive Eligibility: Ready" : "Resting Required"}
                </span>
                <div className="my-3 text-left">
                  {isEligible ? (
                    <div>
                      <p className="text-2xl font-black text-gray-900 dark:text-white">Eligible & Active</p>
                      <p className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 mt-1">90 Day Intervallic Safety Clear</p>
                    </div>
                  ) : (
                    <div>
                      <p className="text-2xl font-black text-amber-500">{remainingDays} Days Left</p>
                      <p className="text-xs font-semibold text-gray-500 dark:text-gray-300 mt-1">Transfusion interval rest period</p>
                    </div>
                  )}
                </div>
                <p className="text-[10px] text-gray-400 dark:text-gray-300">Mandatory clinical interval.</p>
              </div>

              {/* Badge Milestone achievements */}
              <div className="bg-white dark:bg-[#1F2937] rounded-3xl p-6 border border-gray-150 dark:border-gray-800 shadow-md flex flex-col justify-between transition-colors">
                <span className="text-[10px] font-bold text-amber-500 uppercase tracking-widest">Accredited Badges</span>
                <div className="my-3 space-y-1 text-left">
                  {profile.badges && profile.badges.length > 0 ? (
                    <div className="flex flex-wrap gap-1">
                      {profile.badges.map((badge: string, bidx: number) => (
                        <span key={bidx} className="text-[9px] bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300 font-black px-1.5 py-0.5 rounded border border-amber-100 dark:border-amber-900">
                          ⭐ {badge}
                        </span>
                      ))}
                    </div>
                  ) : (
                    <p className="text-xs text-gray-400">Complete donations to earn milestones!</p>
                  )}
                </div>
                <p className="text-[10px] text-gray-400 dark:text-gray-300">Official LifeSaver Milestones.</p>
              </div>
              ...
              <div className="lg:col-span-2 space-y-4 text-left">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-bold text-gray-900 dark:text-white text-lg">Active Matching Emergency Broadcasts</h3>
                  </div>
                  <span className="text-xs font-semibold text-gray-400 font-mono pr-2">{alerts.length} matching Chennai</span>
                </div>
                ...
                {alerts.length === 0 ? (
                  <div className="bg-white dark:bg-[#1F2937] rounded-3xl p-12 border border-gray-150 dark:border-gray-800 text-center text-gray-400 dark:text-gray-300 shadow">
                    No active emergency requirements matched with your O-negative category in the Adyar sector at present.
                  </div>
                ) : (
                    ...
                    {alerts.map((alert) => (
                      <div
                        ...
                        className={`bg-white dark:bg-[#1F2937] rounded-2xl border p-5 flex flex-col justify-between hover:shadow-lg transition relative ${
                          alert.urgency === 'Critical' ? 'border-red-100 dark:border-red-900 ring-2 ring-red-500/10' : 'border-gray-150 dark:border-gray-800'
                        }`}
                      >
                        {alert.urgency === 'Critical' && (
                          <span className="absolute -top-2.5 -right-2.5 px-2 py-0.5 bg-red-600 text-white font-black text-[9px] rounded-full uppercase shadow">CRITICAL</span>
                        )}

                        <div>
                          <div className="flex items-center justify-between">
                            <span className="text-xs text-gray-400 font-mono">Case: {alert.patientReference}</span>
                          ...
                          <h4 className="font-extrabold text-gray-900 dark:text-white mt-2.5 leading-snug">{alert.hospitalName}</h4>
                          <p className="text-xs text-gray-400 mt-0.5 font-semibold flex items-center gap-1"><MapPin size={10} /> {alert.city}</p>
                        ...
                        <div className="mt-5 pt-4 border-t border-gray-50 dark:border-gray-750 flex items-center gap-2">
                          <button
                            ...
                            disabled={!isEligible || !profile.isAvailable || acceptingId !== null}
              ...
              <div className="space-y-4 text-left">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-bold text-gray-900 dark:text-white text-lg">My Alert Inbox</h3>
                  </div>
                ...
                <div className="bg-white dark:bg-[#1F2937] rounded-3xl border border-gray-150 dark:border-gray-800 shadow-xl p-4 divide-y divide-gray-50 dark:divide-gray-750 max-h-[360px] overflow-y-auto transition-colors">
                  {notifications.length === 0 ? (
                    <div className="py-12 text-center text-gray-400 dark:text-gray-300 text-xs">
                      Inbox is empty. Notifications appear here when hospitals request you or register accepts.
                    </div>
                  ) : (
                    notifications.map((not) => (
                      <div key={not.id} className={`py-3 first:pt-0 last:pb-0 ${!not.read ? 'bg-red-500/5 px-2 rounded-lg' : ''}`}>
                        <div className="flex items-center justify-between mb-1">
                          <span className={`text-[8px] font-bold tracking-wider px-2 py-0.5 rounded uppercase bg-red-100 dark:bg-red-950 text-red-650 dark:text-red-400`}>
                            {not.type || "system"}
                          </span>
                          <span className="text-[10px] text-gray-400">{not.createdAt ? new Date(not.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : "just now"}</span>
                        </div>
                        <p className="text-xs font-bold text-gray-900 dark:text-white leading-tight">{not.title}</p>
                        <p className="text-[11px] text-gray-400 dark:text-gray-350 leading-relaxed mt-0.5">{not.message}</p>
                      </div>
                    ))
                  )}
                </div>
            ...
            <div className="bg-white dark:bg-[#1F2937] rounded-3xl p-6 border border-gray-150 dark:border-gray-800 shadow-xl text-left space-y-4 transition-colors animate-in slide-in-from-bottom-2">
              <h3 className="font-bold text-gray-900 dark:text-white text-lg">My Historical Donation Achievements</h3>
              
              {history.length === 0 ? (
                <div className="py-12 text-center text-gray-400 dark:text-gray-300 text-xs">
                  No verified donations recorded under this account passport scorecard. Accept hospital emergencies to record milestones and download certificate credentials!
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-xs text-left text-gray-500 dark:text-gray-300">
                    <thead className="text-[10px] text-gray-400 dark:text-gray-300 uppercase tracking-widest border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/40">
                      <tr>
                        <th scope="col" className="px-6 py-3 font-semibold">Incident/Hospital</th>
                        <th scope="col" className="px-6 py-3 font-semibold">Transfused Blood</th>
                        <th scope="col" className="px-6 py-3 font-semibold">Quantity</th>
                        <th scope="col" className="px-6 py-3 font-semibold">Fulfillment Date</th>
                        <th scope="col" className="px-6 py-3 font-semibold">Credentials verification</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-50 dark:divide-gray-750">
                      {history.map((record) => (
                        <tr key={record.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/30">
                          <td className="px-6 py-4 font-bold text-gray-800 dark:text-white">{record.hospitalName}</td>
                          <td className="px-6 py-4">
                            <span className="px-2 py-0.5 bg-red-105 bg-red-650 text-white rounded text-xs font-black">{record.bloodGroup}</span>
                          </td>
                          <td className="px-6 py-4 font-bold text-emerald-600 dark:text-emerald-400">{record.units} Pint</td>
          ...
          <div className="space-y-8 animate-in fade-in duration-200 text-left">
            ...
            <div className="bg-white dark:bg-[#1F2937] p-6 rounded-3xl border border-gray-150 dark:border-gray-800 shadow-xl space-y-4 transition-colors">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-300">Generate, view and print official accredited LifeSaver badges linked to completed hospital requirements.</p>
                </div>
                {history.length > 0 && (
                  <button
              ...
              {history.length === 0 ? (
                <div className="py-12 text-center text-gray-400 dark:text-gray-300 border border-gray-100 dark:border-gray-700 rounded-2xl">
                  ⚠️ Certificates become claimable immediately once you accept and complete a donation drive at an accredited hospital.
                </div>
              ) : (
                <div className="p-6 bg-red-650/5 border border-red-500/10 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-6">
                  <div className="space-y-1.5">
                    <h4 className="font-extrabold text-sm text-gray-900 dark:text-white">Gold LifeDrop Accreditation Badge</h4>
                    <p className="text-xs text-gray-500 dark:text-gray-300 leading-normal">Our algorithms verified clinical records matching <strong>{profile.fullName}</strong>. Certificate file compiled on June 9, 2026.</p>
                    <span className="text-[10px] font-mono text-gray-400 block">Serial Hash: LD-VERIFY-{donorId.substring(0, 6).toUpperCase()}-2026</span>
                  </div>
                  <button
                    onClick={() => setIsCertOpen(true)}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
              ...
              <div className="bg-white dark:bg-[#1F2937] p-6 rounded-3xl border border-gray-150 dark:border-gray-800 shadow-xl space-y-3 transition-colors">
                ...
                <p className="text-[11px] text-gray-400 leading-normal">Actual log registry of direct e-mails pushed to client inbox.</p>
                
                <div className="space-y-3 max-h-[300px] overflow-y-auto divide-y divide-gray-50 dark:divide-gray-750 pr-1">
                  {emailLogs.length === 0 ? (
                    <p className="text-center text-gray-400 dark:text-gray-300 py-8 text-xs">No email packets sent to {profile.email} yet.</p>
                  ) : (
                    emailLogs.map((mail) => (
              ...
              <div className="bg-white dark:bg-[#1F2937] p-6 rounded-3xl border border-gray-150 dark:border-gray-800 shadow-xl space-y-3 transition-colors">
                ...
                <p className="text-[11px] text-gray-400 leading-normal">Actual log of text dispatches pushed to {profile.phoneNumber}.</p>
                
                <div className="space-y-3 max-h-[300px] overflow-y-auto divide-y divide-gray-50 dark:divide-gray-750 pr-1">
                  {smsLogs.length === 0 ? (
                    <p className="text-center text-gray-400 dark:text-gray-300 py-8 text-xs">No SMS alerts pushed to {profile.phoneNumber} yet.</p>
                  ) : (
                    smsLogs.map((sms) => (
                      <div key={sms.id} className="pt-3 first:pt-0">
                        <div className="flex justify-between items-center mb-1 text-[10px] text-gray-400 dark:text-gray-350">
                          <span>Phone: <strong>{sms.to}</strong></span>
                          <span className="text-emerald-500 font-bold uppercase text-[9px] bg-slate-50 dark:bg-slate-800 p-1 rounded">{sms.status}</span>
                        </div>
        ...
        {activeTab === 'settings' && (
          <div className="max-w-xl mx-auto bg-white dark:bg-[#1F2937] p-6 md:p-8 rounded-3xl border border-gray-150 dark:border-gray-800 shadow-xl space-y-6 text-left animate-in fade-in duration-200 transition-colors">
            <div>
              <h3 className="font-extrabold text-gray-900 dark:text-white text-lg flex items-center gap-2"><Settings size={18} className="text-red-650" /> Alerts & Communications channels</h3>
              <p className="text-xs text-gray-500 dark:text-gray-330 mt-1">Configure enabled channels to prevent matching alarms from waking or overwhelming your schedule.</p>
            </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-850/30 rounded-2xl border border-gray-100 dark:border-gray-700">
                  <div className="space-y-0.5 pr-4">
                    <label className="block text-sm font-extrabold text-gray-900 dark:text-white">Email alerts</label>
                    <span className="text-xs text-gray-400 dark:text-gray-350 leading-relaxed">Sends details of urgent nearby cases matching your type to your e-mail address.</span>
                  </div>
                  <button
                    type="button"
                ...
                <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-850/30 rounded-2xl border border-gray-100 dark:border-gray-700">
                  <div className="space-y-0.5 pr-4">
                    <label className="block text-sm font-extrabold text-gray-900 dark:text-white">Mobile SMS alerts</label>
                    <span className="text-xs text-gray-400 dark:text-gray-350 leading-relaxed">Transmits real-time alert codes directly to your matching Indian mobile line.</span>
                  </div>
                  <button
                    type="button"
